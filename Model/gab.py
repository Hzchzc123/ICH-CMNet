...
class group_aggregation_bridge(nn.Module):
    ...
    def forward(self, xh, xl, mask):
        ...
        mask = F.interpolate(mask.float(), size=[xl.size(2), xl.size(3)], mode='bilinear', align_corners=True)
...
if __name__ == '__main__':
    # 创建模拟输入数据
    xh = torch.randn(1, 128, 128, 128) # 输入 xh 的形状为 [B C H/2 W2]
    xl = torch.randn(1, 256, 256, 256) # 输入 xl 的形状为 [B C H W]
    mask = torch.randn(1, 1,512, 512) # 蒙版张量的形状为 [B 1 H/2 W/2]
    # 实例化模块
    block = group_aggregation_bridge(dim_xh=128, dim_xl=256)
